...
def read_paf(file_path, is_input_paf):
    ...
    return mapped_pairs, unmapped_pairs, bases, mt_ms
...
def compute_throughput(bases, mt_ms):
    if not bases:
    ...
    return mean_bps, median_bps, mean_mt, median_mt

def process_files(input_path, minimap2_path):
    input_mapped, input_unmapped, bases, mt_ms = read_paf(input_path, True)
    minimap2_mapped, minimap2_unmapped, _, _ = read_paf(minimap2_path, False)
    ...
    mean_bps, median_bps, mean_mt, median_mt = compute_throughput(bases, mt_ms)
