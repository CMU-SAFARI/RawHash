...
def check_mapping(truth_info, feature_info):
    ...
    if tstrand == strand and tchr == chr:
        ts_shift = max(1, trs - tqs)
        te_shift = tre + (tqe - tqs)

        s_shift = max(1, rs - qs)
        e_shift = re + (qe - qs)

        # Correct if feature position is within ±3000 base pair range of truth start position
        if (max(ts_shift, s_shift) <= min(te_shift, e_shift)):
            return 1
    return 0
